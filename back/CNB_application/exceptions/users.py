...
class UserNotFound(UserError):
    def __init__(self):
        UserError.__init__(
            self, 'No account associated with this email address', 404)
...
class NoMembershipForSelectedDate(UserError):
    def __init__(self):
        UserError.__init__(
            self,
            'No active membership at the given date for the family',
            400,
        )
...
class AddressAlreadyCreatedForThisAccount(UserError):
    def __init__(self):
        UserError.__init__(
            self, 'Address already created for this family', 400)


class DrinkCardAlreadyCreatedForThisAccount(UserError):
    def __init__(self):
        UserError.__init__(
            self, 'DrinkCard already created for this family', 400)
...
class EmailNotConfirmed(UserError):
    def __init__(self):
        UserError.__init__(
            self,
            "An account with this email address already exists and hasn't been activated",
            401,
        )
