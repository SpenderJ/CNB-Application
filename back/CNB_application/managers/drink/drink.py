...
def get_all_drink_cards() -> list[Drink]:
    ...
    logger.debug(
        "Get all drink objects from db. Number of drink objects : {}".format(
            len(drinks)
        )
    )
...
def create_drink(
    family: Family, drinks_left: Optional[int] = 0, number_of_card: Optional[int] = 0
) -> Drink:
    drink_card = Drink.create(
        family=family, drinks_left=drinks_left, number_of_card=number_of_card
    )
    drink_card.save()
    return Drink
...
def update_drink(
    family_id: str, drinks_left: Optional[int], number_of_card: Optional[int]
) -> Drink:
